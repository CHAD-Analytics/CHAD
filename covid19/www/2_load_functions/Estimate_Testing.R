Estimate_Testing <- function(CONUSSelect,ChosenBase,IncludedCounties){
  ...
  if (CONUSSelect == "CONUS"){
    ...
    PT <- CovidCountiesCases$People_Tested*PopRatio  
    PT <- sum(PT)  
    ...
    StPop <- sum(StPopList$Population)
    PopRatio <- RegPop/StPop
    CovidCountiesCases<-subset(GlobalActive, Country %in% IncludedCounties$Country) 
    
    PT <- CovidCountiesCases$total_tested*PopRatio
    PT <- sum(PT)
  } 
  
  PT<-round(PT, digits = 0)
  Final <- PT  
